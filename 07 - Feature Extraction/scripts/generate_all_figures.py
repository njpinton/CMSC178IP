"""
Master script to generate all figures for Feature Extraction module.
...
def main():
    """Generate all figures for the Feature Extraction module."""
    print("="*60)
    print("GENERATING ALL FEATURE EXTRACTION FIGURES")
    print("="*60)
    ...
    for script_name, module_name in scripts:
        ...
        try:
            ...
            if hasattr(module, '__name__') and module.__name__ == '__main__':
                # Re-execute the module's main code
                exec(compile(open(f"{module_name}.py").read(), f"{module_name}.py", 'exec'))
            else:
